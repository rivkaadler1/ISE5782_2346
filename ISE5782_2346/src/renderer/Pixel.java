...
class Pixel {
	...
	public boolean nextPixel()
	{
		...
		{
			if (cRow == maxRows)//crow is initialized to zero so the last row is maxRows-1
				return false;
			++cCol;//It is initialized to zero
			if (cCol < maxCols) 
			{
				row = cRow;
				col = cCol;
